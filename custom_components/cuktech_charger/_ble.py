...
def _env_hex_bytes(name, default_hex, expected_len):
    ...
    value = os.environ.get(name, default_hex).strip()
    try:
...
class CuktechBLEController:
    ...
    async def _recv_auth_response(self, channel, label="数据"):
        ...
        if data[2] == 0x02:
            ...
            data_id = data[3]
            print(f"    [*] {label}: 多帧模式, {frame_count} 帧, ID=0x{data_id:02x}")
    ...
    async def read_device_info(self):
        ...
        if data:
            ...
            print(f"  协议版本: v{version}.{sub_ver}")
    ...
    async def authenticate(self):
        ...
        Phase B: 密钥交换 (CMD_LOGIN=0x24)
          ...
          5. 收到 0x21 = 登录成功
        """
        print("[*] 开始 MiOT BLE 认证...")

        # 认证通道已在 connect() 中预订阅

        # ---- Phase A: 设备初始化 ----
        print("  [1/5] 设备初始化 (0xa4)...")
        ...
        if key_data:
            print(f"  [*] 密钥交换数据: {len(key_data)} bytes (头: {key_data[:4].hex()})")
        ...
        print("  [2/5] 发送登录命令 (CMD_LOGIN=0x24)...")
        ...
        if not data or data != bytes([0x00, 0x00, 0x01, 0x01]):
            print(f"  [!] 未收到 RCV_RDY, 得到: {data.hex() if data else 'None'}")
            return False
        ...
        if not data or data != bytes([0x00, 0x00, 0x01, 0x00]):
            print(f"  [!] 未收到 RCV_OK, 得到: {data.hex() if data else 'None'}")
            return False
        ...
        if not dev_random or len(dev_random) < 16:
            print(f"  [!] 设备密钥无效: {dev_random.hex() if dev_random else 'None'}")
            return False
        dev_random = dev_random[:16]  # 取前16字节
        print(f"  [*] 设备随机密钥: {dev_random.hex()}")

        # 接收设备 HMAC 信息 (32 字节)
        dev_hmac_info = await self._recv_auth_response("auth_data", "设备HMAC")
        if not dev_hmac_info or len(dev_hmac_info) < 32:
            print(f"  [!] 设备 HMAC 无效: {dev_hmac_info.hex() if dev_hmac_info else 'None'}")
            return False
        dev_hmac_info = dev_hmac_info[:32]  # 取前32字节
        print(f"  [*] 设备 HMAC: {dev_hmac_info.hex()}")
        ...
        if expected_dev_hmac != dev_hmac_info:
            print(f"  [!] 设备 HMAC 验证失败!")
            print(f"      预期: {expected_dev_hmac.hex()}")
            print(f"      实际: {dev_hmac_info.hex()}")
            return False
        print(f"  [+] 设备 HMAC 验证通过!")
    ...
    async def _drain_device_push(self):
        ...
        while True:
            ...
            if push_count >= max_push_count:
                print(f"  [*] 初始化推送达到上限({max_push_count})，停止排空")
                break
            if asyncio.get_event_loop().time() - start >= max_drain_seconds:
                print(f"  [*] 初始化推送排空超时({max_drain_seconds:.1f}s)，继续后续流程")
                break
    ...
    async def _send_encrypted(self, plaintext):
        ...
        if data != bytes([0x00, 0x00, 0x01, 0x01]):
            print(f"  [!] CMD_SEND 未收到 RCV_RDY: {data.hex() if data else 'None'}")
            return False
        ...
        if data != bytes([0x00, 0x00, 0x01, 0x00]):
            print(f"  [!] CMD_SEND 未收到 RCV_OK: {data.hex() if data else 'None'}")
            return False
    ...
    async def send_miot_command(self, siid, piid, value=None):
        ...
        if value is not None:
            ...
            print(f"  [>] SET siid={siid} piid={piid} value={value}")
        else:
            # GET property: opcode=0x02
            plaintext = bytes([0x0c, 0x20, seq, 0x00,
                               0x02, 0x01, siid & 0xFF, piid & 0xFF,
                               0x00, 0x01, 0x10, 0x00])
            print(f"  [>] GET siid={siid} piid={piid}")
        ...
        while True:
            ...
            if data[2] == 0x02 and len(data) >= 4:
                ...
                if b4 == 0x01 and pt_siid == (siid & 0xFF) and pt_piid == (piid & 0xFF):
                    ...
                    print(f"  [<] SET 确认: value={result_value}")
        ...
        while True:
            ...
            if data[2] == 0x02 and len(data) >= 4:
                ...
                if b4 == 0x03 and pt_siid == (siid & 0xFF) and pt_piid == (piid & 0xFF):
                    ...
                    print(f"  [<] GET 响应: value={result_value}")
                    return {'piid': piid, 'value': result_value, 'raw': pt}
                else:
                    # 推送通知 (跳过, 延长超时)
